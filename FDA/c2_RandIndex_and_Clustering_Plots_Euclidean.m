...
Euclid_clust_source = append(join([wp_dir, "Images", "Clustering Images Euclidean"], char_split), char_split);
Euclid_distance_matrix_source = join([wp_dir, "Data Files", "Euclidean Distance Matrices-csv", "euclid_dist_matrix_"], char_split);
...
for i = ix_partial_ims
    ...
    X1 = readtable(append(Euclid_distance_matrix_source,uni_images_wo_aoi(i), ".csv"),'PreserveVariableNames', true);
    ...
    hold off
    title_name = strcat('mds_', uni_images_wo_aoi(i), '_true_euclidean','.png');
    exportgraphics(gcf, append(Euclid_clust_source, title_name))
    ...
    xticks(axis_tics); yticks(axis_tics);
    hold off
    title_name = strcat('mds_', uni_images_wo_aoi(i), '_hc_euclidean','.png');
    exportgraphics(gcf, append(Euclid_clust_source, title_name))
    hold off
...
save(append(mat_path, 'Rand_index_mat_Euclid'), 'Rand_index_mat')
